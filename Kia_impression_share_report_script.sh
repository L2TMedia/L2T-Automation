...
REPORT_NAME="Kia_Impression_Share_Report_$YEAR-$MONTH.CSV"
...
(SELECT ActiveClientsCliName, ActiveClientsCliId, ActiveClientsGrossBudget, ActiveClientsNetBudget FROM ActiveClients WHERE ActiveClientsYear in ($YEAR) and ActiveClientsMonth in ($MONTH) and ActiveClientsProgramId in (20) and ActiveClientsProductId in (9)) as AC
